...
struct titsc {
	...
	u32			charge_delay;
	int 		sample_cnt;
	u32			samp_x[SAMPLE_MAX_COUNT];
	u32			samp_y[SAMPLE_MAX_COUNT];
	u32			samp_x_min, samp_x_max;
	u32			samp_y_min, samp_y_max;
};
...
static irqreturn_t titsc_irq(int irq, void *dev)
{
	...
	if (ts_dev->pen_down == false) {
		ts_dev->sample_cnt = 0;
		ts_dev->samp_x_min = ts_dev->samp_y_min = ~0;
		ts_dev->samp_x_max = ts_dev->samp_y_max = 0;
	}
	...
	if (status & IRQENB_FIFO0THRES) {
		...
		if (ts_dev->pen_down && z1 != 0 && z2 != 0) {
			...
			if (z <= MAX_12BIT) {
				if (ts_dev->sample_cnt >= SAMPLE_MAX_COUNT - 1) {
					u32 sum_x = 0;
					u32 sum_y = 0;
					int i;
					for (i = 0; i < SAMPLE_MAX_COUNT; i++) {
						sum_x += ts_dev->samp_x[i];
						sum_y += ts_dev->samp_y[i];
					}
					x = (sum_x - ts_dev->samp_x_min - ts_dev->samp_x_max) / (SAMPLE_MAX_COUNT - 2);
					y = (sum_y - ts_dev->samp_y_min - ts_dev->samp_y_max) / (SAMPLE_MAX_COUNT - 2);
					...
					int i = ts_dev->sample_cnt;
					ts_dev->samp_x[i] = x;
					ts_dev->samp_y[i] = y;
					if (x < ts_dev->samp_x_min)
						ts_dev->samp_x_min = x;
					if (x > ts_dev->samp_x_max)
						ts_dev->samp_x_max = x;
					if (y < ts_dev->samp_y_min)
						ts_dev->samp_y_min = y;
					if (y > ts_dev->samp_y_max)
						ts_dev->samp_y_max = y;
					ts_dev->sample_cnt++;
				}
			}
